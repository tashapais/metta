...
def test_event_v17_depositor_staging_details_are_serializable():
    details = event_v17_depositor_staging_details()

    assert details["name"] == "event_v17_depositor_staging"
    assert details["role_names"] == list(EVENT_V17_DEPOSITOR_STAGING_ROLE_NAMES)
    assert details["v17_changes"]["target_aware_handoff_mask"] is True
    assert details["v17_changes"]["reward_penalties_added"] is False
    assert details["v17_changes"]["scripted_policy_added"] is False
    assert "depositor_empty_move_toward_home" in details["final_mile_action_coefficients"]


def test_event_v17_rewards_empty_depositor_staging_beyond_v16():
    stats = np.zeros((3, len(SIMULATOR_STAT_COLUMNS)), dtype=np.float64)
    before = np.zeros((3, len(NAVIGATION_SNAPSHOT_COLUMNS)), dtype=np.float64)
    after = before.copy()
    before[:, [NAV_AGENT_X, NAV_AGENT_Y, NAV_HOME_ASSEMBLER_X, NAV_HOME_ASSEMBLER_Y]] = [5, 5, 8, 5]
    ...
    assert v17[:2].tolist() == pytest.approx(v16[:2].tolist())
    assert v17[2] - v16[2] == pytest.approx(
        EVENT_V17_DEPOSITOR_STAGING_ACTION_COEFFICIENTS["depositor_empty_move_toward_home"]
    )


def test_event_v17_does_not_add_empty_staging_when_depositor_has_battery():
    stats = np.zeros((3, len(SIMULATOR_STAT_COLUMNS)), dtype=np.float64)
    before = np.zeros((3, len(NAVIGATION_SNAPSHOT_COLUMNS)), dtype=np.float64)
    after = before.copy()
    before[:, [NAV_AGENT_X, NAV_AGENT_Y, NAV_HOME_ASSEMBLER_X, NAV_HOME_ASSEMBLER_Y]] = [5, 5, 8, 5]
...
def test_event_v19_crafter_home_delivery_details_are_serializable():
    details = event_v19_crafter_home_delivery_details()

    assert details["name"] == "event_v19_crafter_home_delivery"
    assert details["role_names"] == list(EVENT_V19_CRAFTER_HOME_DELIVERY_ROLE_NAMES)
    assert details["v19_changes"]["target_aware_handoff_mask"] is True
    assert details["v19_changes"]["reward_penalties_added"] is False
    assert details["v19_changes"]["scripted_policy_added"] is False
    assert "crafter_battery_move_toward_home" in details["final_mile_action_coefficients"]


def test_event_v19_rewards_battery_crafter_home_delivery_beyond_v17():
    stats = np.zeros((3, len(SIMULATOR_STAT_COLUMNS)), dtype=np.float64)
    before = np.zeros((3, len(NAVIGATION_SNAPSHOT_COLUMNS)), dtype=np.float64)
    after = before.copy()
    before[:, [NAV_AGENT_X, NAV_AGENT_Y, NAV_HOME_ASSEMBLER_X, NAV_HOME_ASSEMBLER_Y]] = [5, 5, 8, 5]
    ...
    assert v19[0] == pytest.approx(v17[0])
    assert v19[1] - v17[1] == pytest.approx(
        EVENT_V19_CRAFTER_HOME_DELIVERY_ACTION_COEFFICIENTS["crafter_battery_move_toward_home"]
    )
    assert v19[2] == pytest.approx(v17[2])


def test_event_v19_does_not_reward_empty_crafter_home_delivery():
    stats = np.zeros((3, len(SIMULATOR_STAT_COLUMNS)), dtype=np.float64)
    before = np.zeros((3, len(NAVIGATION_SNAPSHOT_COLUMNS)), dtype=np.float64)
    after = before.copy()
    before[:, [NAV_AGENT_X, NAV_AGENT_Y, NAV_HOME_ASSEMBLER_X, NAV_HOME_ASSEMBLER_Y]] = [5, 5, 8, 5]
...
def test_event_v21_home_handoff_rendezvous_details_are_serializable():
    details = event_v21_home_handoff_rendezvous_details()

    assert details["name"] == "event_v21_home_handoff_rendezvous"
    assert details["role_names"] == list(EVENT_V21_HOME_HANDOFF_RENDEZVOUS_ROLE_NAMES)
    assert details["v21_changes"]["target_aware_handoff_mask"] is True
    assert details["v21_changes"]["reward_penalties_added"] is False
    assert details["v21_changes"]["scripted_policy_added"] is False
    assert details["v21_changes"]["changes_action_permissions"] is False
    assert "crafter_battery_move_toward_home_staged_depositor" in details["final_mile_action_coefficients"]


def test_event_v21_strengthens_masked_home_staged_handoff_beyond_v20():
    stats = np.zeros((3, len(SIMULATOR_STAT_COLUMNS)), dtype=np.float64)
    before = np.zeros((3, len(NAVIGATION_SNAPSHOT_COLUMNS)), dtype=np.float64)
    after = before.copy()
    before[:, [NAV_AGENT_X, NAV_AGENT_Y, NAV_HOME_ASSEMBLER_X, NAV_HOME_ASSEMBLER_Y]] = [6, 5, 8, 5]
    ...
    v21 = event_v21_home_handoff_rendezvous_bonuses(
        stats,
        stats,
        navigation_before=before,
        navigation_after=after,
    ...
    assert v21[1] - v20[1] == pytest.approx(
        EVENT_V21_HOME_HANDOFF_RENDEZVOUS_ACTION_COEFFICIENTS[
            "crafter_battery_put_to_home_staged_depositor"
        ]
        - EVENT_V20_HOME_STAGED_HANDOFF_ACTION_COEFFICIENTS[
...
def test_event_v21_rewards_crafter_move_to_home_staged_depositor_beyond_v20():
    stats = np.zeros((3, len(SIMULATOR_STAT_COLUMNS)), dtype=np.float64)
    before = np.zeros((3, len(NAVIGATION_SNAPSHOT_COLUMNS)), dtype=np.float64)
    after = before.copy()
    before[:, [NAV_HOME_ASSEMBLER_X, NAV_HOME_ASSEMBLER_Y]] = [8, 5]
    after[:, [NAV_HOME_ASSEMBLER_X, NAV_HOME_ASSEMBLER_Y]] = [8, 5]
    before[1, [NAV_AGENT_X, NAV_AGENT_Y, NAV_INVENTORY_BATTERY, NAV_DIST_HOME_ASSEMBLER]] = [5, 5, 1, 3]
    after[1, [NAV_AGENT_X, NAV_AGENT_Y, NAV_INVENTORY_BATTERY, NAV_DIST_HOME_ASSEMBLER]] = [7, 5, 1, 1]
    before[2, [NAV_AGENT_X, NAV_AGENT_Y, NAV_DIST_HOME_ASSEMBLER]] = [8, 5, 0]
    after[2, [NAV_AGENT_X, NAV_AGENT_Y, NAV_DIST_HOME_ASSEMBLER]] = [8, 5, 0]
    actions = np.array(
        [
            MOVE_VERB * ACTION_ARGUMENT_COUNT + 3,
            MOVE_VERB * ACTION_ARGUMENT_COUNT + 3,
    ...
    assert v21[0] == pytest.approx(v20[0])
    assert v21[1] - v20[1] == pytest.approx(expected)
    assert v21[2] == pytest.approx(v20[2])


def test_event_v21_requires_home_staged_depositor_for_rendezvous():
    stats = np.zeros((3, len(SIMULATOR_STAT_COLUMNS)), dtype=np.float64)
    before = np.zeros((3, len(NAVIGATION_SNAPSHOT_COLUMNS)), dtype=np.float64)
    after = before.copy()
    before[:, [NAV_HOME_ASSEMBLER_X, NAV_HOME_ASSEMBLER_Y]] = [8, 5]
    after[:, [NAV_HOME_ASSEMBLER_X, NAV_HOME_ASSEMBLER_Y]] = [8, 5]
    before[1, [NAV_AGENT_X, NAV_AGENT_Y, NAV_INVENTORY_BATTERY, NAV_DIST_HOME_ASSEMBLER]] = [5, 5, 1, 3]
    after[1, [NAV_AGENT_X, NAV_AGENT_Y, NAV_INVENTORY_BATTERY, NAV_DIST_HOME_ASSEMBLER]] = [6, 5, 1, 2]
    before[2, [NAV_AGENT_X, NAV_AGENT_Y, NAV_DIST_HOME_ASSEMBLER]] = [8, 5, 3]
    after[2, [NAV_AGENT_X, NAV_AGENT_Y, NAV_DIST_HOME_ASSEMBLER]] = [8, 5, 3]
    actions = np.array([MOVE_VERB * ACTION_ARGUMENT_COUNT + 3] * 3, dtype=np.int64)
    action_mask = np.ones((3, 56), dtype=bool)
    ...
    np.testing.assert_allclose(v21, v20)


def test_event_v22_v17_targeted_put_details_are_serializable():
    details = event_v22_v17_targeted_put_details()

    assert details["name"] == "event_v22_v17_targeted_put"
    assert details["role_names"] == list(EVENT_V22_V17_TARGETED_PUT_ROLE_NAMES)
    assert details["v22_changes"]["target_aware_handoff_mask"] is True
    assert details["v22_changes"]["reward_penalties_added"] is False
    assert details["v22_changes"]["scripted_policy_added"] is False
    assert details["v22_changes"]["changes_action_permissions"] is False
    assert details["v22_changes"]["uses_home_delivery_breadcrumbs"] is False
    assert "crafter_battery_put_to_targeted_depositor" in details["oracle_action_coefficients"]


def test_event_v22_rewards_masked_targeted_put_beyond_v17_without_home_requirement():
    stats = np.zeros((3, len(SIMULATOR_STAT_COLUMNS)), dtype=np.float64)
    before = np.zeros((3, len(NAVIGATION_SNAPSHOT_COLUMNS)), dtype=np.float64)
    after = before.copy()
    before[:, [NAV_AGENT_X, NAV_AGENT_Y, NAV_HOME_ASSEMBLER_X, NAV_HOME_ASSEMBLER_Y]] = [3, 5, 8, 5]
    after[:, [NAV_AGENT_X, NAV_AGENT_Y, NAV_HOME_ASSEMBLER_X, NAV_HOME_ASSEMBLER_Y]] = [3, 5, 8, 5]
    ...
    assert v22[0] == pytest.approx(v17[0])
    assert v22[1] - v17[1] == pytest.approx(
        EVENT_V22_V17_TARGETED_PUT_ACTION_COEFFICIENTS[
            "crafter_battery_put_to_targeted_depositor"
        ]
    )
    assert v22[2] == pytest.approx(v17[2])


def test_event_v22_requires_masked_put_and_battery_crafter():
    stats = np.zeros((3, len(SIMULATOR_STAT_COLUMNS)), dtype=np.float64)
    before = np.zeros((3, len(NAVIGATION_SNAPSHOT_COLUMNS)), dtype=np.float64)
    after = before.copy()
    before[:, [NAV_AGENT_X, NAV_AGENT_Y, NAV_HOME_ASSEMBLER_X, NAV_HOME_ASSEMBLER_Y]] = [3, 5, 8, 5]
    ...
    no_battery_v17 = event_v17_depositor_staging_bonuses(
        stats,
        stats,
        navigation_before=before,
        navigation_after=after,
    ...
    no_battery_v22 = event_v22_v17_targeted_put_bonuses(
        stats,
        stats,
        navigation_before=before,
        navigation_after=after,
    ...
    after[1, NAV_INVENTORY_BATTERY] = 1
    action_mask[1, put_east] = False
    masked_v17 = event_v17_depositor_staging_bonuses(
        stats,
        stats,
        navigation_before=before,
        navigation_after=after,
    ...
    masked_v22 = event_v22_v17_targeted_put_bonuses(
        stats,
        stats,
        navigation_before=before,
        navigation_after=after,
...
def test_checkpoint_chain_affordance_mask_infers_legacy_v10_config():
    ...
    assert _checkpoint_uses_chain_affordance_action_mask(
        {
            "reward_design": "event_v22_v17_targeted_put",
            "chain_affordance_action_mask": False,
        }
    )

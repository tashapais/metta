...
from v3_experiments.tribal_event_rewards import (  # noqa: E402
    ...
    EVENT_V15_DEPOSITOR_FINAL_MILE_ROLE_COEFFICIENTS,
    ...
    EVENT_V17_DEPOSITOR_STAGING_ROLE_COEFFICIENTS,
    ...
    EVENT_V18_HANDOFF_RENDEZVOUS_ROLE_COEFFICIENTS,
    ...
    EVENT_V19_CRAFTER_HOME_DELIVERY_ROLE_COEFFICIENTS,
    ...
    EVENT_V20_HOME_STAGED_HANDOFF_ROLE_COEFFICIENTS,
    ...
    EVENT_V23_V17_DEPOSITOR_RENDEZVOUS_ROLE_COEFFICIENTS,
    EVENT_V23_V17_DEPOSITOR_RENDEZVOUS_ROLE_NAMES,
    EVENT_V24_V17_HANDOFF_POTENTIAL_ACTION_CAPS,
    EVENT_V24_V17_HANDOFF_POTENTIAL_ACTION_COEFFICIENTS,
    EVENT_V24_V17_HANDOFF_POTENTIAL_ROLE_COEFFICIENTS,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=(
            ...
            "event_v24_v17_handoff_potential",
        ),
        default="passive_v0",
...
def _role_shaping_bonuses_for_design(
    ...
    if config.reward_design == "event_v15_depositor_final_mile":
        return event_v15_depositor_final_mile_bonuses(
            event_stats_delta,
            event_stats_total,
            navigation_before=navigation_before,
            navigation_after=navigation_after,
    ...
    if config.reward_design == "event_v16_handoff_reliability":
        return event_v16_handoff_reliability_bonuses(
            event_stats_delta,
            event_stats_total,
            navigation_before=navigation_before,
            navigation_after=navigation_after,
    ...
    if config.reward_design == "event_v17_depositor_staging":
        return event_v17_depositor_staging_bonuses(
            event_stats_delta,
            event_stats_total,
            navigation_before=navigation_before,
            navigation_after=navigation_after,
    ...
    if config.reward_design == "event_v18_handoff_rendezvous":
        return event_v18_handoff_rendezvous_bonuses(
            event_stats_delta,
            event_stats_total,
            navigation_before=navigation_before,
            navigation_after=navigation_after,
    ...
    if config.reward_design == "event_v19_crafter_home_delivery":
        return event_v19_crafter_home_delivery_bonuses(
            event_stats_delta,
            event_stats_total,
            navigation_before=navigation_before,
            navigation_after=navigation_after,
    ...
    if config.reward_design == "event_v20_home_staged_handoff":
        return event_v20_home_staged_handoff_bonuses(
            event_stats_delta,
            event_stats_total,
            navigation_before=navigation_before,
            navigation_after=navigation_after,
    ...
    if config.reward_design == "event_v21_home_handoff_rendezvous":
        return event_v21_home_handoff_rendezvous_bonuses(
            event_stats_delta,
            event_stats_total,
            navigation_before=navigation_before,
            navigation_after=navigation_after,
    ...
    if config.reward_design == "event_v23_v17_depositor_rendezvous":
        return event_v23_v17_depositor_rendezvous_bonuses(
            event_stats_delta,
            event_stats_total,
            navigation_before=navigation_before,
            navigation_after=navigation_after,
    ...
    if config.reward_design == "event_v24_v17_handoff_potential":
        return event_v24_v17_handoff_potential_bonuses(
            event_stats_delta,
            event_stats_total,
            navigation_before=navigation_before,
            navigation_after=navigation_after,
...
def _uses_target_aware_handoff_mask(config: RunnerConfig) -> bool:
    return _uses_chain_affordance_action_mask(config) and config.reward_design in (
        "event_v14_target_aware_handoffs",
        "event_v15_depositor_final_mile",
        "event_v16_handoff_reliability",
        "event_v17_depositor_staging",
...
def _reward_design_role_names(config: RunnerConfig) -> list[str]:
    ...
    if config.reward_design == "event_v15_depositor_final_mile":
        return list(EVENT_V15_DEPOSITOR_FINAL_MILE_ROLE_NAMES)
    if config.reward_design == "event_v16_handoff_reliability":
        return list(EVENT_V16_HANDOFF_RELIABILITY_ROLE_NAMES)
    if config.reward_design == "event_v17_depositor_staging":
        return list(EVENT_V17_DEPOSITOR_STAGING_ROLE_NAMES)
    if config.reward_design == "event_v18_handoff_rendezvous":
        return list(EVENT_V18_HANDOFF_RENDEZVOUS_ROLE_NAMES)
    if config.reward_design == "event_v19_crafter_home_delivery":
        return list(EVENT_V19_CRAFTER_HOME_DELIVERY_ROLE_NAMES)
    if config.reward_design == "event_v20_home_staged_handoff":
    ...
    if config.reward_design == "event_v23_v17_depositor_rendezvous":
        return list(EVENT_V23_V17_DEPOSITOR_RENDEZVOUS_ROLE_NAMES)
    if config.reward_design == "event_v24_v17_handoff_potential":
        return list(EVENT_V24_V17_HANDOFF_POTENTIAL_ROLE_NAMES)
...
def _reward_design_coefficients(config: RunnerConfig) -> dict[str, Any]:
    ...
    if config.reward_design == "event_v15_depositor_final_mile":
        return {
            ...
            "oracle_actions": dict(EVENT_V13_ROLE_GATED_DEPOSITOR_USE_ACTION_COEFFICIENTS),
            "final_mile_actions": dict(EVENT_V15_DEPOSITOR_FINAL_MILE_ACTION_COEFFICIENTS),
            ...
            "roles": {
                role: dict(coefficients)
                for role, coefficients in EVENT_V15_DEPOSITOR_FINAL_MILE_ROLE_COEFFICIENTS.items()
            },
        }
    if config.reward_design == "event_v16_handoff_reliability":
        return {
            ...
            "roles": {
                role: dict(coefficients)
                for role, coefficients in EVENT_V16_HANDOFF_RELIABILITY_ROLE_COEFFICIENTS.items()
            },
        }
    if config.reward_design == "event_v17_depositor_staging":
        return {
            ...
            "roles": {
                role: dict(coefficients) for role, coefficients in EVENT_V17_DEPOSITOR_STAGING_ROLE_COEFFICIENTS.items()
            },
        }
    if config.reward_design == "event_v18_handoff_rendezvous":
        return {
            ...
            "roles": {
                role: dict(coefficients)
                for role, coefficients in EVENT_V18_HANDOFF_RENDEZVOUS_ROLE_COEFFICIENTS.items()
            },
        }
    if config.reward_design == "event_v19_crafter_home_delivery":
        return {
            ...
            "roles": {
                role: dict(coefficients)
                for role, coefficients in EVENT_V19_CRAFTER_HOME_DELIVERY_ROLE_COEFFICIENTS.items()
            },
        }
    if config.reward_design == "event_v20_home_staged_handoff":
        return {
            ...
            "roles": {
                role: dict(coefficients)
                for role, coefficients in EVENT_V20_HOME_STAGED_HANDOFF_ROLE_COEFFICIENTS.items()
            },
        }
    if config.reward_design == "event_v21_home_handoff_rendezvous":
        return {
    ...
    if config.reward_design == "event_v23_v17_depositor_rendezvous":
        return {
            ...
            "roles": {
                role: dict(coefficients)
                for role, coefficients in EVENT_V23_V17_DEPOSITOR_RENDEZVOUS_ROLE_COEFFICIENTS.items()
            },
        }
    if config.reward_design == "event_v24_v17_handoff_potential":
        return {
            ...
            "potential_shaping": {
                ...
                "stage_offsets": dict(EVENT_V12_ROLE_GATED_DEPOSITOR_RELIABILITY_STAGE_OFFSETS),
                "target_closeness_scales": dict(EVENT_V12_ROLE_GATED_DEPOSITOR_RELIABILITY_CLOSENESS_SCALES),
                "crafter_battery_target": "nearest_empty_depositor_manhattan_fallback_home_assembler",
            },
            "oracle_actions": {
                "depositor_use_home_assembler": EVENT_V24_V17_HANDOFF_POTENTIAL_ACTION_COEFFICIENTS[
                    "depositor_use_home_assembler"
                ],
            },
            "final_mile_actions": {
                name: coefficient
                for name, coefficient in EVENT_V24_V17_HANDOFF_POTENTIAL_ACTION_COEFFICIENTS.items()
                if name != "depositor_use_home_assembler"
            },
            ...
            "roles": {
                role: dict(coefficients)
                for role, coefficients in EVENT_V24_V17_HANDOFF_POTENTIAL_ROLE_COEFFICIENTS.items()
            },
        }
...
def _reward_design_details(config: RunnerConfig) -> dict[str, Any]:
    ...
    if config.reward_design == "event_v15_depositor_final_mile":
        ...
        if not _uses_chain_affordance_action_mask(config):
            ...
            details["action_affordance_curriculum"]["blocked_successes"] = []
            details["action_affordance_curriculum"]["purpose"] = (
                "Relax the v15 target-aware role-gated mask for transfer/ablation diagnostics."
            )
        return details
    if config.reward_design == "event_v16_handoff_reliability":
        ...
        if not _uses_chain_affordance_action_mask(config):
            ...
            details["action_affordance_curriculum"]["blocked_successes"] = []
            details["action_affordance_curriculum"]["purpose"] = (
                "Relax the v16 target-aware role-gated mask for transfer/ablation diagnostics."
            )
        return details
    if config.reward_design == "event_v17_depositor_staging":
        ...
        if not _uses_chain_affordance_action_mask(config):
            ...
            details["action_affordance_curriculum"]["blocked_successes"] = []
            details["action_affordance_curriculum"]["purpose"] = (
                "Relax the v17 target-aware role-gated mask for transfer/ablation diagnostics."
            )
        return details
    if config.reward_design == "event_v18_handoff_rendezvous":
        ...
        if not _uses_chain_affordance_action_mask(config):
            ...
            details["action_affordance_curriculum"]["blocked_successes"] = []
            details["action_affordance_curriculum"]["purpose"] = (
                "Relax the v18 target-aware role-gated mask for transfer/ablation diagnostics."
            )
        return details
    if config.reward_design == "event_v19_crafter_home_delivery":
        ...
        if not _uses_chain_affordance_action_mask(config):
            ...
            details["action_affordance_curriculum"]["blocked_successes"] = []
            details["action_affordance_curriculum"]["purpose"] = (
                "Relax the v19 target-aware role-gated mask for transfer/ablation diagnostics."
            )
        return details
    if config.reward_design == "event_v20_home_staged_handoff":
        ...
        if not _uses_chain_affordance_action_mask(config):
            ...
            details["action_affordance_curriculum"]["blocked_successes"] = []
            details["action_affordance_curriculum"]["purpose"] = (
                "Relax the v20 target-aware role-gated mask for transfer/ablation diagnostics."
            )
        return details
    if config.reward_design == "event_v21_home_handoff_rendezvous":
    ...
    if config.reward_design == "event_v23_v17_depositor_rendezvous":
        ...
        if not _uses_chain_affordance_action_mask(config):
            ...
            details["action_affordance_curriculum"]["blocked_successes"] = []
            details["action_affordance_curriculum"]["purpose"] = (
                "Relax the v23 target-aware role-gated mask for transfer/ablation diagnostics."
            )
        return details
    if config.reward_design == "event_v24_v17_handoff_potential":
        ...
        if not _uses_chain_affordance_action_mask(config):
            ...
            details["action_affordance_curriculum"]["blocked_successes"] = []
            details["action_affordance_curriculum"]["purpose"] = (
                "Relax the v24 target-aware role-gated mask for transfer/ablation diagnostics."
            )
        return details
    return {
        "name": "passive_v0",
